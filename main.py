...
#                        if np.isnan(df.loc[pdbid,f"""{measurement["label"]}bin{i}"""]):
#                            all_measured = False
...
#                    if measurement["use_bins"]:
#                        for i in range(len(measurement["bins"])):
#                            features.append(df.loc[pdbid,f"""{measurement["label"]}bin{i}"""])
#                    else:
#                        features.append(df.loc[pdbid, measurement["label"]])
#
        features_np = np.array(features)

        # features = extract_features_by_description(protein, ligand, feature_descriptions)
...
def main(arguments):
    ...
    all_df = all_df[0:1] # TODO: use multiple complexes
    # change types
...
#    else:
    ...
    if num_cores == 1:
        all_features = compute_features(all_features, pro_elements, pro_ele_rad, directory, lig_elements,lig_ele_rad, feature_descriptions)
#    else:
